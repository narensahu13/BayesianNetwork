...
hideAllBut = function(divList, butNdx) {
  library("shinyjs")
  divList[-butNdx] %>% sapply(function(x) {
    shinyjs::hide(x)
  })
...
add_parent_child_rel <- function(network, parent_ID, child_ID) {
  if (all(c(parent_ID, child_ID) %in% names(network))) {
    network[[parent_ID]]$Children <-
      c(child_ID, network[[parent_ID]]$Children) %>% unique
    network[[child_ID]]$Parents   <-
      c(parent_ID, network[[child_ID]]$Parents) %>% unique
...
get_parent_child_table <- function(network) {
  ...
  sapply(parents, function(parent_ID) {
    ...
    tibble::tibble(Parent = rep(parent_ID, length(children), Child = children))
  }) %>% do.call(what = 'rbind')
}
...
delete_node <- function(network, node_ID) {
  if (node_ID %in% names(network)) {
    ...
    for (parent in network[[node_ID]]$Parents) {
      network[[node_ID]]$Children <-
        network[[parent]]$Children %>% Filter(
          f = function(v)
            v != node_ID
        )
...
mapping_Hydenet_network <- function(network) {
  strng <-
    sapply(network, mapping_node, type = 'Hydenet') %>% paste(collapse = '+')
    ...
    if (!(new_state %in% network[[node_ID]]$States)) {
      if (new_state != '') {
        ...
        network[[node_ID]]$States <-
          union(new_state, network[[node_ID]]$States)
      }
    }
  }
  network
...
remove_state_from_node <- function(network, node_ID, old_state) {
  if (node_ID %in% names(network)) {
    if (old_state %in% network[[node_ID]]$States) {
      ...
      network[[node_ID]]$States <-
        setdiff(network[[node_ID]]$States, old_state)
    }
  }
  network
}
...
plot_hydenet <- function(net) {
  if (any(c('Exposure', 'Occurence', 'Impact') %in% net[['nodes']])) {
    net <- HydeNet::setUtilityNodes(net, 'Exposure')
    net <- HydeNet::setUtilityNodes(net, 'Occurence')
    net <- HydeNet::setUtilityNodes(net, 'Impact')
  }
  return(plot(net))
  
}

## get all the states in a node
...
check_state <- function(network) {
  ...
  for (node_ID in setdiff(names(network), remove)) {
    ...
    if (node_type == 'Numeric') {
      check <-
        network[[node_ID]]$States %>% as.vector %>% as.double %>% is.na %>% unique
      if (length(check) == 1) {
        if (check == TRUE) {
          return(paste(
            "Numeric node contains non-numeric states, check NodeID",
            node_ID
          ))
        }
      } else {
      ...
      check <-
        network[[node_ID]]$States %>% toupper %!in% c('TRUE', 'FALSE') %>% unique
      if (length(check) == 1) {
        if (check == TRUE) {
          return(paste(
            "Boolean node contains undefined states, check Node ID:",
            node_ID
          ))
        }
      } else {
...
calc_CPT_structure_for_node <- function(network, node_ID) {
  ...
  if (node_ID %in% names(network)) {
    ...
    if (network[[node_ID]]$IS_ROOT) {
      dimnams <-
        structure(list(network[[node_ID]]$States), .Names = node_ID)
      CPT     <-
        array(1 / node_n_states, dim = node_n_states, dimnames = dimnams)
    } else {
      parent_states <-
        network[[node_ID]]$Parents %>% lapply(function(nodenam)
          network[[nodenam]]$States)
      parent_n_states <- parent_states %>% sapply(length)
      dimnams <-
        structure(list(network[[node_ID]]$States), .Names = node_ID)
      dimnams <-
        c(dimnams,
          structure(parent_states, .Names = network[[node_ID]]$Parents))
      CPT <-
        array(
          1 / node_n_states,
          dim = c(node_n_states, parent_n_states),
          dimnames = dimnams
        )
    }
  }
...
node_without_CPT <- function(network) {
  remove <- c('Exposure', 'Occurence', 'Impact')
  non_CPT_node <- c()
  for (id in names(network) %>% setdiff(remove)) {
    if (is.null(network[[id]]$CPT)) {
      non_CPT_node <- c(non_CPT_node, id)
    }
  }
  return(non_CPT_node)
}
    ...
    if (!is.null(network[[node_ID]]$Parents)) {
      ...
      if (length(parents_type) == 1) {
        if (parents_type == 'Numeric') {
          ...
          parent_states <-
            network[[node_ID]]$Parents %>% lapply(function(nodenam)
              network[[nodenam]]$States %>% as.double)
          dimnams <-
            structure(parent_states, .Names = network[[node_ID]]$Parents)
          check <- anyNA(dimnams %>% unlist)
          if (!check) {
            network[[node_ID]]$States <-
              dimnams %>% as.vector %>% expand.grid %>% as.data.frame %>% 
              dplyr::mutate(states = eval(parse(text = formula))) %>%
              dplyr::select(states) %>% as.vector %>% format(scientific = F) %>%
              unlist %>% unique %>% as.double %>% sort
            CPT <-
              calc_CPT_structure_for_node(network, node_ID) %>% transform_CPT
            ...
            if (ncol(CPT) == 1) {
              ...
              for (i in 2:ncol(CPT)) {
                ...
                for (j in 1:n_parents) {
                  assign(rev(network[[node_ID]]$Parents)[j],
                         CPT[j, i] %>% as.character %>% as.double)
                } %>% as.double(row.names = F)
                value <-
                  eval(parse(text = formula))# %>% format(scientific=F) %>% as.double
            ...
            if (n_dims >= 3) {
              CPT_array <-
                tail.matrix(CPT[-1], length(network[[node_ID]]$States)) %>% 
                as.array %>% unlist %>% as.double %>% c
            }
            #CPT_array <- tail.matrix(CPT[-1], length(network[[node_ID]]$States) ) %>% 
            as.array  %>% unlist %>% as.double %>% c
...
update_cpt_to_network <- function(network, node_ID, CPT_array) {
  if (node_ID %in% names(network)) {
    ...
    if (n_dims == 1 | n_dims == 2) {
      transformed_cpt <-
        tail.matrix(CPT_array, length(network[[node_ID]]$States))
    }
    if (n_dims >= 3) {
      transformed_cpt <-
        tail.matrix(CPT_array[-1], length(network[[node_ID]]$States)) %>% as.array %>% unlist %>% as.double %>% c
    }
    ...
    if (!(is.null(network[[node_ID]]))) {
      if (!(is.null(network[[node_ID]]$CPT))) {
        if (network[[node_ID]]$CPT %>% as.data.frame %>% colSums %>% sum != prod(dim(network[[node_ID]]$CPT)[-1])) {
          ...
          if (length(dims) == 1) {
            network[[node_ID]]$CPT <-
              network[[node_ID]]$CPT / network[[node_ID]]$CPT %>% sum
          } else if (length(dims) == 2) {
            network[[node_ID]]$CPT <-
              (network[[node_ID]]$CPT %>% t / network[[node_ID]]$CPT %>% as.data.frame %>% colSums) %>% t
          }
          else if (length(dims) >= 3) {
            network[[node_ID]]$CPT[1:prod(dim(network[[node_ID]]$CPT))] <-
              (
                network[[node_ID]]$CPT %>% as.data.frame %>% t /
                  network[[node_ID]]$CPT %>% as.data.frame %>% colSums
              ) %>% t
          }
        }
      }
...
transform_CPT <- function(CPT) {
  ...
  if (n_dims == 1) {
    ...
    for (i in 2:n_dims) {
      assign(paste0('parent_states_', i - 1), dimnames(CPT)[[i]])
    }
    for (i in 2:n_dims) {
      assign(paste0('parent_name_', i - 1), names(dimnames(CPT)[i]))
    }
    matrix_dim <- c(dims[1] + n_dims - 1, prod(dims[-1]) + 1)
    df <-
      data.frame(matrix(NA, nrow = matrix_dim[1], ncol = matrix_dim[2]))
    if (n_dims  == 3) {
      df[, 1] <- c(parent_name_2, parent_name_1, child_states)
      df[2, -1] <-
        rep(parent_states_1, time = prod(dims[-1]) / dims[2])
      df[1, -1] <-
        rep(parent_states_2, each = prod(dims[-1]) / dims[3])
    } else if (n_dims == 4) {
      df[, 1] <- c(parent_name_3,
                   parent_name_2,
                   parent_name_1,
                   child_states)
      df[3, -1] <-
        rep(parent_states_1, time = prod(dims[-1]) / dims[2])
      df[2, -1] <-
        rep(parent_states_2, each = prod(dims[-1]) / dims[3] / dims[4]) %>% rep(time =
                                                                                  dims[4])
      df[1, -1] <-
        rep(parent_states_3, each = prod(dims[-1]) / dims[4])
    } else if (n_dims == 5) {
      df[, 1] <-
        c(parent_name_4,
          parent_name_3,
      ...
      df[4, -1] <-
        rep(parent_states_1, time = prod(dims[-1]) / dims[2])
      df[3, -1] <-
        rep(parent_states_2, each = prod(dims[-1]) / dims[3] / dims[4]) %>% rep(time = dims[4] /
                                                                                  dims[5]) %>% rep(time = dims[5])
      df[2, -1] <-
        rep(parent_states_3, each = prod(dims[-1]) / dims[4] / dims[5]) %>% rep(time = dims[5])
      df[1, -1] <-
        rep(parent_states_4, each = prod(dims[-1]) / dims[5])
    } else if (n_dims == 6) {
      df[, 1] <-
        c(
          parent_name_5,
      ...
      df[4, -1] <-
        rep(parent_states_2, each = prod(dims[-1]) / dims[3] / dims[4]) %>% rep(time = dims[4] /
                                                                                  dims[5]) %>% rep(time = dims[5] / dims[6]) %>% rep(time = dims[6])
      df[3, -1] <-
        rep(parent_states_3, each = prod(dims[-1]) / dims[4] / dims[5]) %>% rep(time = dims[5] /
                                                                                  dims[6]) %>% rep(time = dims[6])
      df[2, -1] <-
        rep(parent_states_4, each = prod(dims[-1]) / dims[5] / dims[6]) %>% rep(time = dims[6])
      df[1, -1] <-
        rep(parent_states_5, each = prod(dims[-1]) / dims[6])
    }
...
get_formula_node <- function(network) {
  formula_node_ID <- c()
  for (id in names(network)) {
    if (network[[id]]$IS_ROOT == FALSE) {
      formula_node_ID <- c(formula_node_ID, id)
    }
  }
  remove <- c('Exposure', 'Occurence', 'Impact')
...
check_formula <- function(formula, network, node_ID) {
  if (node_ID %in% names(network)) {
    ...
    check <- res[is.na(suppressWarnings(as.numeric(res)))] %>% 
...
net_transform <- function(network) {
  ...
  if (c('Exposure', 'Occurence', 'Impact')  %in%  names(network_sim) %>% unique == TRUE) {
    ...
    parents_used <-
      c(
        network_sim$Exposure$Parents,
        network_sim$Occurence$Parents,
        network_sim$Impact$Parents
      )
...
RepParallel <- function(n, expr, simplify = "array", ...) {
  answer <-
    parallel::mclapply(integer(n), eval.parent(substitute(function(...)
      expr)), ...)
...
do_simulation <-
  function(network,
           ...
           formula_impact) {
    parents_used <-
      c(network$Exposure$Parents,
        network$Occurence$Parents,
        network$Impact$Parents)
    parent_occurence <- network$Occurence$Parents
    sim <-
      bnlearn::rbn(bnlearn_net_fit, blocksize) #%>% mutate(Iter_vec)
    sim[, parent_occurence] <-
      ifelse(toupper(sim[, parent_occurence]) == TRUE, 1, 0)
    sim[, parents_used] <-
      lapply(sim[, parents_used], function(x)
        as.double(as.vector(x)))
    sim <-
      sim %>% dplyr::mutate(Occurence =  eval(parse(text = formula_occurence)), Impact = eval(parse(text = formula_impact))) %>%
      dplyr::mutate(Loss_tot = Occurence * Impact) %>%
      dplyr::transmute(Occurence, Loss_tot)
  }
...
run_simulation <-
  function(network,
    ...
    bnlearn_net <-
      network_sim %>% mapping_bnlearn_network %>% model2network
    bnlearn_net_fit <-
      bnlearn::custom.fit(bnlearn_net, dist = network_sim %>% lapply(function(v)
        v$CPT))
    ###
    ...
    sim <- do.call("rbind", RepParallel(n_blocks, {
      do_simulation(network,
                    bnlearn_net_fit,
                    blocksize,
                    formula_occurence,
                    formula_impact)
    }, simplify = FALSE)) %>% as.data.frame
    sim <- sim[1:length(Iter_vec), ] %>% dplyr::mutate(Iter_vec)
  ...
  if (node_ID %in% names(network)) {
    if (!is.null(network[[node_ID]]$CPT)) {
      if (is.null(network[[node_ID]]$Parents)) {
        plot <-
          barplot(
            network[[node_ID]]$CPT %>% as.double,
            names.arg = network[[node_ID]]$States,
            xlab = 'Probabilities',
            ylab = node_ID,
            main = paste('Marginal Probabilities for Node', node_ID),
            col = 'darkolivegreen3',
            cex.axis = 1,
            cex.names = 1,
            cex.lab = 1.5,
            border = "red",
            horiz = T,
            xlim = c(0, max(network[[node_ID]]$CPT) + 0.2)
          )
        text(
          y = plot,
          x = network[[node_ID]]$CPT %>% as.double,
          signif(network[[node_ID]]$CPT %>% as.double, 4) ,
          pos = 2,
          labels = network[[node_ID]]$CPT %>% as.double %>% scales::percent(accuracy = 0.01)
        )
      } else {
        ...
        plot <-
          barplot(
            ...
            border = "red",
            horiz = T,
            xlim = c(0, max(prob) + 0.2)
          )
        text(
          y = plot,
          x = prob,
          pos = 4,
          labels = prob %>% scales::percent(accuracy = 0.01)
        )
      }
    }
  }
  ...
  if (node_ID %in% names(network)) {
    if (!is.null(network[[node_ID]]$CPT)) {
      ...
      if (n_dims == 1) {
        ...
        for (i in 1:length(network[[node_ID]]$Parents)) {
          assign(paste0('parent', i), network[[node_ID]]$Parents[i])
        }
        if (n_dims == 2) {
          prob <-
            data.frame(mapply(
              `*`,
              network[[node_ID]]$CPT %>% as.data.frame,
              rep(
                calc_marinal_prob(network, parent1),
                times = prod(dims[-1]) / dims[2]
              ) ,
              SIMPLIFY = FALSE
            )) %>% rowSums
        } else if (n_dims == 3) {
          prob <-
            data.frame(mapply(`*`, network[[node_ID]]$CPT %>% as.data.frame,
                              (
                                rep(
                                  calc_marinal_prob(network, parent1),
                                  times = prod(dims[-1]) / dims[2]
                                ) * rep(
                                  calc_marinal_prob(network, parent2),
                                  each = prod(dims[-1]) / dims[3]
                                )
                              )),
                       SIMPLIFY = FALSE) %>% rowSums
        } else if (n_dims == 4) {
          prob <-
            data.frame(mapply(
              `*`,
              ...
              (
                rep(
                  ...
                  calc_marinal_prob(network, parent3),
                  each = prod(dims[-1]) / dims[4]
                )
              ),
              SIMPLIFY = FALSE
            )) %>% rowSums
            ...
            data.frame(mapply(
              ...
              (
                rep(
                  ...
                  calc_marinal_prob(network, parent4),
                  each = prod(dims[-1]) / dims[5]
                )
              ),
              SIMPLIFY = FALSE
            )) %>% rowSums
            ...
            data.frame(mapply(
              ...
              (
                rep(
                  ...
                  calc_marinal_prob(network, parent5),
                  each = prod(dims[-1]) / dims[6]
                )
              ),
              SIMPLIFY = FALSE
            )) %>% rowSums
...
modFunction <- function(input, output, session, data,reset) {
  v <- shiny::reactiveValues(data = data)
  proxy = DT::dataTableProxy("mod_table")
  shiny::observeEvent(input$mod_table_cell_edit, {
    print(names(v$data))
    ...
    shiny::isolate(if (j %in% match(c("ratio", "cost", "updated_price"), names(v$data))) {
  ...
  print(shiny::isolate(colnames(v$data)))
...
widgetThumbnail <-
  function(p,
    ...
    if (phantom == "") {
      message(
        "** phantomjs dependency could not be found - thumbnail cannot be generated (run phantomInstall() for details)"
      )
    } else {
      res <- try({
        ...
        js <- paste0(
          ...
          height,
          " };
                    page.clipRect = { top: 0, left: 0, width: ",
          width,
          ", height: ",
          height,
          " };
